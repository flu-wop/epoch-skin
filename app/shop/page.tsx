"use client";
// app/shop/page.tsx
// Luxury shop with exact "By Concern" categories as specified.
// Smooth scroll to first matching product on filter click.
...
// ── By Concern mapping — exact IDs as specified ──────────────────
const CONCERNS: { label: string; ids: string[] | "all-except-wax" | "all" }[] = [
...
export default function ShopPage() {
  ...
  const filtered = (() => {
    ...
    return concern.ids.map(id => products.find(p => p.id === id)).filter(Boolean) as typeof products;
  })();
        ...
        <div className="flex flex-col lg:flex-row gap-12">
          ...
          <aside className="lg:w-52 flex-shrink-0">
              ...
              {CONCERNS.map((c) => (
                <li key={c.label}>
                  <button
                    ...
                    className={`w-full flex items-center justify-between py-3 px-1 border-b border-[#E5DCCF]
                                 text-sm font-sans text-left transition-colors duration-300 ${
            ...
            {activeConcern !== "All Products" && (
              <button onClick={() => setActiveConcern("All Products")}
          ...
          <div className="flex-1" ref={gridRef}>
            ...
            {filtered.length === 0 ? (
              <div className="text-center py-20">
                <p className="font-serif text-2xl text-[#1C1C1A] mb-3">No products found</p>
                <button onClick={() => setActiveConcern("All Products")}
                  className="text-[#C9A96E] text-sm font-sans underline">View all</button>
              </div>
            ) : (
                  ...
                  <div key={product.id}
                    ...
                    <Link href={`/shop/${product.slug}`} className="block relative overflow-hidden bg-[#F5F0E8]">
                      ...
                      {["top-3 left-3 border-t border-l","top-3 right-3 border-t border-r",
                        ...
                        <span key={i} className={`absolute w-4 h-4 ${cls} border-[#C9A96E]/0 group-hover:border-[#C9A96E]/60 transition-all duration-500`} />
                      ))}
                    </Link>
                    <div className="p-5 flex flex-col flex-1">
                      <Link href={`/shop/${product.slug}`} className="flex-1">
                        <h3 className="font-serif text-[#1C1C1A] text-lg leading-snug mb-2 group-hover:text-[#C9A96E] transition-colors duration-400">
                          {product.name}
                        </h3>
                      ...
                      <div className="flex items-center justify-between pt-4 border-t border-[#F0EBE0]">
                        <div>
                          ...
                          {product.size && <span className="text-[#C0BAB4] text-[10px] font-sans ml-2">{product.size}</span>}
                        </div>
        ...
        <div className="max-w-[1320px] mx-auto">
          <div className="grid md:grid-cols-3 gap-5">
            {[
              ...
              <div key={item.title} className="border border-[#E5DCCF] p-8 bg-white hover:border-[#C9A96E]/40 transition-colors duration-400">
